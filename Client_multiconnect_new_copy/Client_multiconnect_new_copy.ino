...
void setup() {
  ...
  starMoney.respawnTimer=60;
  starMoney.ledPin=12;
  starMoney.isOnCooldown=false;
...
void loop() {
  ...
  if (connectedServers > 0) {
    for (int i = 0; i < MAX_SERVERS; i++) {
      ...
      if (servers[i].connected&&servers[i].pClient!=nullptr){
      ...
      Serial.print("wave: ");
      Serial.println(checkWaving(servers[i].acc[0], servers[i].acc[1], servers[i].acc[2]));
      if (servers[i].close && checkWaving(servers[i].acc[0], servers[i].acc[1], servers[i].acc[2])){
...
bool checkWaving(float aX, float aY, float aZ){
  Serial.print("X: ");
  Serial.print(aX);
  Serial.print(", Y: ");
  Serial.print(aY);
...
void updateStation(){
  ...
  if (starMoney.hasStar){
    //analogWrite(starMoney.ledPin,random(0,256));
    digitalWrite(starMoney.ledPin,HIGH);
  }
  else{
    digitalWrite(starMoney.ledPin,LOW);
